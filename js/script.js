...
if (localStorage.getItem('wasEdited'))
...
function loadFile(index) {
	...
	reader.onload = (function(theFile) {
		return function(e) {
			...
			document.getElementById('wasEdited').innerHTML = '';
...
function notifyUpdate(content) {
	if (!localStorage.getItem('wasEdited')) {
		localStorage.setItem('wasEdited',true);
		document.getElementById('wasEdited').innerHTML = '*';
	}
...
function updateCK() {
	...
	notifyUpdate(editorText);
}

function updateCM() {
	var sourceText = myCodeMirror.getValue();	
	notifyUpdate(sourceText);
}
...
$('#anotherLink').click(function(evt) {
	if (localStorage.getItem('wasEdited')) {		
		evt.stopPropagation();
		evt.preventDefault();
		var response = confirm("Go, but save the current state?");
		if(response){
